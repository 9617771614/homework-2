...
section .data                      ; секция данных
    ...
    messegCalcEnterNum2 db "after entering the first number, Enter the '+', second number, Enter the '='" , 100 ; Стрпока подтверждения выбора 
...
selectMode:
 ...
 ;   mov rsi, messegSelectFlip       ; вывод сообщения "Подтверждение выбора режима разворота строки "
    ...
    mov rbx, [readMesseg]       ; что сравниваем
    cmp rax, rbx                ; сравнение
    je SelectCalculator        ; если есть флаг совпадения переходим к калькулятора
...
SelectCalculator:
    ...
    ; Если ввели символ Y –Yes значит переходим к функции каоткуоятора
    mov rax, 'Y'                    ; 
    mov rbx, [readMesseg]           ; передаем введеный символ
    cmp rax, rbx                    ; сравниваем символы
    je enterStringForCalculator           ; переход к вводу числа
...
; вывод введеного символа
  ...
  ;  syscall
    ...
    ; првоерка, совпадения введеного символа с символом +
    mov rbx, '+'
 ;   mov rbx, tempData
    cmp  rbx, rax
    je StartEnterNumB              ; переход к функции ввода чилса 2
 ;   jmp enterString
...
; вывод введеного символа
 ...
 ;   jmp enterString
    ...
    mov rax, [count]                ; сравниваем с счетчиком
    cmp  rbx, rax                   ; сравнение
    je StartSum             ; переход к функции калькулятора 
